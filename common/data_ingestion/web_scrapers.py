...
class SimpleWebPageReader(BaseReader):
    ...
    def load_data(self, urls: List[str]) -> List[Document]:
        ...
        for url in urls:
            ...
            response = Preprocessor().remove_special_tokens(response)
    ...
    def load_data(
        ...
        for url in urls:
            ...
            if hostname in self.website_extractor:
                ...
                data = soup.getText()
            data = Preprocessor().remove_special_tokens(data)
            documents.append(Document(data, extra_info=extra_info))

        return documents


class BeautifulSoupWebTraverseReader(BaseReader):
    """BeautifulSoup web page reader.
    ...
    def load_data(
        ...
        while urls:
            ...
            # Add all links on the page to the list of URLs to visit
            for link in soup.find_all("a"):
                href = link.get("href")
                if href and not href.startswith("#"):
                    full_url = urljoin(url, href)
                    if hostname in full_url:
